...
class Neighbor implements Comparable<Neighbor> {
   ...
   // implement all methods needed for a HashSet and TreeSet to work with Neighbor
   // objects
   ...
   // add all methods needed for a HashSet and TreeSet to function with Neighbor
   // objects
   // use only target, not distances, since a vertex can't have 2 neighbors that
...
class PQelement implements Comparable<PQelement> {
   // used just for a PQ, contains a wVertex and a distance, also previous that is
   // used for Dijkstra 7
   // compareTo is using the distanceToVertex to order them such that the
   // PriorityQueue works
   ...
   public int compareTo(PQelement other) {
      // we assume no overflow will happen since distances will not go over the range
      // of int
...
interface wVertexInterface {
   ...
   /*
    * returns an arraylist of PQelements that store distanceToVertex to another
    * wVertex
    */
    ...
    * or the distance associated with that wVertex in case there is a PQelement
    * that has v as wVertex
    */
   public Double getDistanceToVertex(wVertex v);

   /*
    * precondition: v is not null
    * postcondition: - if the alDistanceToVertex has a PQelement that has the
    * wVertex component equal to v
    * it updates the distanceToVertex component to d
    * - if the alDistanceToVertex has no PQelement that has the wVertex component
    * equal to v,
...
public class AdjListWeighted implements AdjListWeightedInterface, AdjListWeightedInterfaceWithCities
{
   ...
   public List<String> getShortestPathTo(wVertex vSource, wVertex target) {
      ...
      while (cur != null) {
         ...
         cur = vSource.getPQelement(cur).getPrevious();
      }
      return path;
   }
   ...
   public void readData(String vertexNames, String edgeListData) {
      /* use a try-catch */
      try {
         Scanner cityScan = new Scanner(new File(vertexNames));
         while (cityScan.hasNext()) {
            addVertex(cityScan.next());
         }

         Scanner edgeScan = new Scanner(new File(edgeListData));
         while (edgeScan.hasNext()) {
            addEdge(edgeScan.next(), edgeScan.next(), edgeScan.nextDouble());
         }
